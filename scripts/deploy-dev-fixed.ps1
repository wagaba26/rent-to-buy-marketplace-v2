...
Write-Host "ðŸš€ Starting Development Deployment..." -ForegroundColor Cyan
Write-Host ""

# Step 1: Setup environment files
Write-Host "ðŸ“ Step 1: Setting up environment files..." -ForegroundColor Yellow
if (-not (Test-Path ".env")) {
    Write-Host "Creating .env files..." -ForegroundColor Cyan
    & "$PSScriptRoot\setup-dev-env.ps1"
} else {
    Write-Host "âœ… .env files already exist" -ForegroundColor Green
}

# Step 2: Check prerequisites
Write-Host ""
Write-Host "ðŸ” Step 2: Checking prerequisites..." -ForegroundColor Yellow
...
try {
    ...
    Write-Host "âŒ Docker not found. Please install Docker Desktop." -ForegroundColor Red
    exit 1
}
...
try {
    ...
    Write-Host "âŒ Docker Compose not found." -ForegroundColor Red
    exit 1
}
...
try {
    ...
    Write-Host "âŒ Node.js not found. Please install Node.js 18+." -ForegroundColor Red
    exit 1
}
...
if (-not $SkipInfrastructure) {
    Write-Host ""
    Write-Host "ðŸ³ Step 3: Starting infrastructure services..." -ForegroundColor Yellow
    ...
    if ($containers.Count -gt 0) {
        ...
        Write-Host "âš ï¸  Warning: No containers found. Check docker-compose logs." -ForegroundColor Yellow
    }
} else {
    Write-Host ""
    Write-Host "â­ï¸  Step 3: Skipping infrastructure (already running)" -ForegroundColor Yellow
}

# Step 4: Install dependencies
if (-not $SkipDependencies) {
    Write-Host ""
    Write-Host "ðŸ“¦ Step 4: Installing dependencies..." -ForegroundColor Yellow
    
    Write-Host "Installing root dependencies..." -ForegroundColor Cyan
    npm install
    
    if ($LASTEXITCODE -ne 0) {
        Write-Host "âŒ Failed to install root dependencies" -ForegroundColor Red
        exit 1
    }
    ...
    foreach ($service in $services) {
        ...
        if (Test-Path $servicePath) {
            ...
            if ($LASTEXITCODE -eq 0) {
                ...
                Write-Host "âš ï¸  Warning: Failed to install $service dependencies" -ForegroundColor Yellow
            }
        }
    }
    ...
    foreach ($lib in $sharedLibs) {
        ...
        if (Test-Path $libPath) {
            ...
            Pop-Location
            if ($LASTEXITCODE -eq 0) {
                Write-Host "âœ… shared/$lib dependencies installed" -ForegroundColor Green
            }
        }
    }
} else {
    Write-Host ""
    Write-Host "â­ï¸  Step 4: Skipping dependency installation" -ForegroundColor Yellow
}

# Step 5: Initialize database
if (-not $SkipDatabase) {
    Write-Host ""
    Write-Host "ðŸ—„ï¸  Step 5: Initializing database..." -ForegroundColor Yellow
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "âš ï¸  Warning: Database initialization may have failed. Check logs." -ForegroundColor Yellow
    }
} else {
    Write-Host ""
    Write-Host "â­ï¸  Step 5: Skipping database initialization" -ForegroundColor Yellow
}

# Step 6: Verify deployment
Write-Host ""
Write-Host "âœ… Step 6: Verifying deployment..." -ForegroundColor Yellow

# Check if Instant.dev App ID is set
$envContent = Get-Content ".env" -Raw
if ($envContent -match "your-instant-app-id-here") {
    Write-Host "âš ï¸  WARNING: Please update NEXT_PUBLIC_INSTANT_APP_ID in .env with your Instant.dev App ID" -ForegroundColor Yellow
}
